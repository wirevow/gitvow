...
def test_session_start_records_state(repo, home, payload):
    ...
    st = json.loads((repo / ".git" / "provkit-session.json").read_text())
...
def test_pre_tool_use_blocks_and_logs(repo, home, payload):
    ...
    log = (repo / ".git" / "provkit-hooks.log").read_text()
...
def test_commit_flow_trailer_note_and_attribution(repo, home, payload, transcript):
    ...
    assert "Provkit-Session: sess-1" in body and "Provkit-Step: 1" in body
    ...
    assert note.startswith("provkit-session")
...
def test_human_commit_has_no_trailer(repo, home):
    ...
    assert "Provkit-Session" not in git(repo, "log", "-1", "--format=%B")

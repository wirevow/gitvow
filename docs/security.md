...
| `kubectl --context prod-in-dp delete pod x` against gitvow 0.17.0 | Allowed. The deny rule was `\bkubectl\s+(delete\|drain\|cordon)\b`, so any option between `kubectl` and its verb walked past it; the same was true of `kubectl … apply`, `helm --kube-context … upgrade`, `terraform -chdir=… destroy` and `aws --profile … terminate-instances`. Found on 2026-09-16 by replaying 15,549 real tool calls from three engineers' sessions through the default policy: 163 cluster mutations, 49 of them deletes against production-named contexts, none of them stopped. Nobody was bypassing anything; the agents were running kubectl the way people run kubectl. | **was bypassed** — closed in 0.17.1: command rules may name a `program` and its `verbs`, and the default rules do; the same commands now deny or confirm |

...
## In a pull request
GitHub does not display git notes. Until a check publishes them, the fastest path is `git fetch origin refs/notes/sessions:refs/notes/sessions` then `provkit show <sha>` locally, or ask the author to paste `provkit show` output into the description.

## Finding sessions
```sh
git log --grep=Provkit-Session:                          # all agent commits
git log --grep='Provkit-Session: <id>' --show-notes=sessions
grep -h session_id ~/.provkit/ledger/*.json | sort | uniq -c
```

...
```sh
gitvow install --user            # writes ~/.gitvow/policy.json if absent
$EDITOR ~/.gitvow/policy.json
gitvow check -- <a command>      # dry-run any rule
```
...
## Validation
gitvow validates the file on every load: every entry needs a `pattern`, every pattern must compile. An invalid file fails closed, refusing all tool calls with the reason, so a typo is loud rather than silent.

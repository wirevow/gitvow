...
src/provkit/
  ...
  state.py        git helpers; .git/provkit-session.json; .git/provkit-hooks.log
...
- `install` twice equals `install` once; `install` then `uninstall` restores the settings file byte-for-byte except provkit's entries.

...
## Session
One Claude Code conversation, identified by the session id Claude Code assigns. provkit records it in the repository's `.git` directory when the session starts, so git hooks can see it. It never enters the working tree.
...
```sh
git log --grep=Provkit-Session:                 # every agent-made commit
git log --grep='Provkit-Session: 8f3d5c71'      # everything one session produced
```
...
Notes are git objects but not part of the tree. `git status`, diffs and checkouts never see them. They are local until pushed with `git push origin refs/notes/sessions`, and read with `git log --show-notes=sessions` or `provkit show <commit>`.

## Ledger
At the end of a session, a redacted summary is written to `~/.provkit/ledger/<session-id>.json`: every tool call with a shortened argument, the commits made during the session, the last stated plan. It is the fullest record provkit keeps, and it never enters a repository. See [What stays out of git](storage.md).

...
The agent's transcript contains tool output: file contents, query results, environment variables, and whatever a database returned. It must not be committed to a repository. provkit is built around that constraint.
...
| Ledger: redacted tool calls, commits, plan | `~/.provkit/ledger/` | no |
| Hook log: every allow, confirm, deny | `<repo>/.git/provkit-hooks.log` | no, `.git` is never pushed |
| Session state | `<repo>/.git/provkit-session.json` | no |
...
4. **An engineering record.** Ledger events as facts next to a derived model of the codebase, so "why" is queryable beside "what". Redacted only. Out of provkit's scope; provkit is the source such a record consumes.

...
**Who made this change, what were they trying to do, and was it allowed?** For code written with AI agents, git alone cannot answer. provkit makes it answer, using only git and a few hooks.
...
It is not a transcript store. Transcripts contain tool output, and tool output contains whatever the agent read. provkit records structure and redacted intent, and leaves the content on the machine that produced it. See [What stays out of git](concepts/storage.md).

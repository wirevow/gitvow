...
if [ -f "$STATE" ]; then
  ...
  if ! grep -q "^Gitvow-\\(Accepted\\|Declined\\|Open\\|Referred\\|Observed\\):" "$1"; then
...
for f in st.get("findings") or []:
    d = f.get("decision")
    if not d and f.get("immediate"):
        continue  # an immediate confirm nobody answered: the command never ran, nothing to record
    if not d:
        out.append(("Gitvow-Observed: " if f.get("observe") else "Gitvow-Open: ") + f["finding"]); continue
